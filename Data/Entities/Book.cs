...
namespace BookBazaar.Entities
{
    ...
    public class Book
    {
        ...
        [NotMapped]
        [Display(Name = "MLA Citation")]
        public string MlaCitation
        {
            get
            {
                string authorName = Author?.FullName ?? (Author?.LastName + " " + Author?.FirstName) ?? "Unknown Author";
                return $"{authorName}. \"{Title}\". ISBN: {ISBN}.";
            }
        }

        // Chicago-style citation
        [NotMapped]
        [Display(Name = "Chicago Citation")]
